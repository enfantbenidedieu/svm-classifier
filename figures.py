...
def serve_roc_curve(model, X_test, y_test):
    ...
    data = [trace0]
    figure = go.Figure(data=data, layout=layout)
    figure.add_shape(type='line', line=dict(dash='dash'),x0=0, x1=1, y0=0, y1=1)
    figure.add_shape(type='line', line=dict(dash='dash'),x0=1, x1=1, y0=0, y1=1)
    figure.add_shape(type='line', line=dict(dash='dash'),x0=0, x1=1, y0=1, y1=1)
    figure.update_yaxes(scaleanchor="x", scaleratio=1)
    figure.update_xaxes(constrain='domain')

    return figure

def serve_lift_curve(model,xtest,ytest):
    score = model.predict_proba(xtest)[:,1]
    pos = np.array(pd.get_dummies(ytest))[:,1]
    npos = np.sum(pos)
    ...
    trace = go.Scatter(x=taille, y=rappel, mode="lines", name="Test Data", marker={"color": "#13c6e9"})
    layout = go.Layout(title="Lift Curve",xaxis=dict(title="Percentage of sample", gridcolor="#2f3445"),
                        yaxis=dict(title="Gain", gridcolor="#2f3445"),legend=dict(x=0, y=1.05, orientation="h"),
                        margin=dict(l=5, r=15, t=40, b=40),
                        plot_bgcolor="rgba(0, 0, 0, 0)",
                        paper_bgcolor="rgba(0, 0, 0, 0)",
                        font={"color": "#a5b1cd"}
                    )
